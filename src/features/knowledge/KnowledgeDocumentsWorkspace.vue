...
<template>
  <div v-if="!kbIdValid" class="gk-root" :data-testid="testId ?? 'knowledge-docs-workspace'">
    <div class="gk-banner gk-banner--error">知识库 ID 无效。</div>
    <button class="gk-btn" type="button" @click="goBack">{{ backLabel }}</button>
  </div>

  <div v-else class="gk-root" :data-testid="testId ?? 'knowledge-docs-workspace'">
    <header class="gk-doc-head">
      <button class="gk-back-btn" type="button" @click="goBack">{{ backLabel }}</button>
